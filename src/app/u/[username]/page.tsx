...
import { Star, MessageCircle, Share2, Globe, Twitter, Github, Zap } from "lucide-react";
...
export default function SellerProfilePage({ params: paramsPromise }: { params: Promise<{ username: string }> }) {
  ...
  const sellerPrompts = prompts.filter(p => p.seller === params.username);
  const totalSales = sellerPrompts.reduce((acc, p) => acc + p.sales, 0);
      ...
      <div className="space-y-16">
        <div className="glass-panel p-12 md:p-16 rounded-[4rem] border-white/5 relative overflow-hidden shadow-3xl">
          ...
          <div className="flex flex-col md:flex-row items-center md:items-start gap-12 relative z-10">
            <div className="w-44 h-44 rounded-[3rem] overflow-hidden border-2 border-skyblue/20 p-1.5 shadow-2xl group">
              <img 
                src={user.avatar} 
